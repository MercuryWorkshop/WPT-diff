...
(function (global_scope) {
  ...
  WindowTestEnvironment.prototype.on_tests_ready = function () {
    ...
    add_completion_callback(function (tests, harness_status, asserts_run) {
      ...
      window.collectWPTResults(window.location.href, tests, harness_status, asserts_run)
  ...
  ShadowRealmTestEnvironment.prototype.begin = function (message_destination) {
    ...
    var fakeMessagePort = {
      postMessage: message_destination
    };
  ...
  function promise_rejects_dom(test, type, promiseOrConstructor, descriptionOrPromise, maybeDescription) {
    ...
    if (typeof promiseOrConstructor === "function" && promiseOrConstructor.name === "DOMException") {
      ...
      assert(maybeDescription === undefined, "Too many args passed to no-constructor version of promise_rejects_dom, or accidentally explicitly passed undefined");
    }
    return bring_promise_to_current_realm(promise).then(test.unreached_func("Should have rejected: " + description)).catch(function (e) {
    ...
    switch (typeof val) {
      ...
      default:
        try {
          ...
          return "[stringifying object threw " + String(e) + " with type " + String(typeof e) + "]";
        }
    }
  }
  ...
  function assert_equals(actual, expected, description) {
    ...
    if (typeof actual != typeof expected) {
      assert(false, "assert_equals", description, "expected (" + typeof expected + ") ${expected} but got (" + typeof actual + ") ${actual}", {
        expected: expected,
        actual: actual
      });
      return;
  ...
  function assert_array_equals(actual, expected, description) {
    ...
    assert(typeof actual === "object" && actual !== null && "length" in actual, "assert_array_equals", description, "value is ${actual}, expected array", {
      actual: actual
    });
  ...
  function assert_between_exclusive(actual, lower, upper, description) {
    ...
    assert(typeof lower === typeof upper, "assert_between_exclusive", description, "expected lower (${type_lower}) and upper (${type_upper}) types to match (test error)", {
      type_lower: typeof lower,
      type_upper: typeof upper
    });
  ...
  function _assert_inherits(name) {
    return function (object, property_name, description) {
      assert(typeof object === "object" && object !== null || typeof object === "function" ||
  ...
  function assert_readonly(object, property_name, description) {
    ...
    if (desc.hasOwnProperty("value")) {
      ...
      assert(desc.writable === false, "assert_readonly", description, "descriptor [[Writable]] expected false got ${actual}", {
  ...
  function assert_throws_js_impl(constructor, func, description, assertion_type) {
    try {
      ...
      assert(typeof e === "object", assertion_type, description, "${func} threw ${e} with type ${type}, not an object", {
        func: func,
        e: e,
        type: typeof e
      });
   ...
   *
   */
  Test.prototype.step_wait_func = function (cond, func, description, timeout = 3000, interval = 100) {
   ...
   *
   */
  Test.prototype.step_wait_func_done = function (cond, func, description, timeout = 3000, interval = 100) {
  ...
  function cleanup_done(test, errors, bad_value_count) {
    if (errors.length || bad_value_count) {
      ...
      if (errors.length) {
        ...
        tests.status.stack = typeof errors[0] === "object" && errors[0].hasOwnProperty("stack") ? errors[0].stack : null;
      }
      if (bad_value_count) {
  ...
  Tests.prototype.all_done = function () {
    return (this.tests.length > 0 || this.remotes.length > 0) && test_environment.all_loaded && (this.num_pending === 0 || this.is_aborted) && !this.wait_for_finish && !this.processing_callbacks && !this.remotes.some(function (w) {
      return w.running;
    });
  };
  ...
  */
  ...
  Output.prototype.show_results = function (tests, harness_status, asserts_run) {
    ...
    function get_asserts_output(test) {
      const asserts_output = render(["details", {}, ["summary", {}, "Asserts run"], ["table", {}, ""]]);
      ...
      for (const assert of asserts) {
        ...
        table.appendChild(render(["tr", {
          class: "overall-" + status_class_name
        }, ["td", {
          class: status_class_name
        }, Test.prototype.status_formats[assert.status]], ["td", {}, ["pre", {}, ["strong", {}, assert.assert_name], output_fn]]]));
      }
      return asserts_output;
    }
    var assertions = has_assertions();
    const section = render(["section", {}, ["h2", {}, "Details"], ["table", {
      id: "results",
      class: assertions ? "assertions" : ""
    }, ["thead", {}, ["tr", {}, ["th", {}, "Result"], ["th", {}, "Test Name"], assertions ? ["th", {}, "Assertion"] : "", ["th", {}, "Message"]]], ["tbody", {}]]]);
    const tbody = section.querySelector("tbody");
    for (const test of tests) {
      const status = test.format_status();
      const status_class_name = status_class(status);
      tbody.appendChild(render(["tr", {
        class: "overall-" + status_class_name
      }, ["td", {
        class: status_class_name
      }, status], ["td", {}, test.name], assertions ? ["td", {}, get_assertion(test)] : "", ["td", {}, test.message ?? "", ["pre", {}, test.stack ?? ""]]]));
  ...
  if (global_scope.addEventListener) {
    function error_handler(error, message, stack) {
